...
# save file to CSV
area_summary_table.to_csv(os.path.join(dirName, str("SEM_mito_area_stats_by_animal.csv")))

# For mito Ferets, get the summary stats for each layer from each animal
Feret_summary_table = mito_data[["Feret_diam_um", "Animal", "Layer"]].groupby(["Animal", "Layer"]).describe()

# save file to CSV
Feret_summary_table.to_csv(os.path.join(dirName, str("SEM_mito_diam_stats_by_animal.csv")))

# For mito count, get the summary stats for each layer from each animal
count_summary_table = mito_avgs[["Count", "Animal", "Layer"]].groupby(["Animal", "Layer"]).describe()

# save file to CSV
count_summary_table.to_csv(os.path.join(dirName, str("SEM_mito_count_stats_by_animal.csv")))
